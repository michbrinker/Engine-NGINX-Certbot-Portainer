...
services:
  certbot_dns_duckdns:
    ...
    entrypoint: "/bin/sh -c \"trap exit TERM; while :; do certbot renew --post-hook 'chmod 744 /etc/letsencrypt && chmod 644 -R /etc/letsencrypt/archive/$jks_domain/fullchain*.pem && && chmod 644 -R /etc/letsencrypt/archive/$jks_domain/privkey*.pem'; sleep 12h & wait $${!}; done;\""
